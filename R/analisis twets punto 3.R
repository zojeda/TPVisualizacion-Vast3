...
tweets_tidy$periodo=c(rep('antes',56282))
#antes 56282
#despues 51699
...
wordcloud_custom <- function(grupo, df){
  print(grupo)
  wordcloud(words = df$token, freq = df$frecuencia, scale=c(2,.05),
            max.words = 80, random.order = FALSE, rot.per = 0.35,
            colors = brewer.pal(10, "Dark2"))
}

df_grouped <- tweets_tidy %>% group_by(periodo, token) %>% count(token) %>%
...
frec_palabra<-tweets_tidy %>% group_by(token) %>% count(token) %>%
  top_n(10, n) %>% arrange(desc(n)) %>% print(n=30)

library(xlsx)
setwd("C:/Users/Melo/Desktop/")
write.csv(frec_palabra, file = "frec_palabra_antes.csv", row.names = F)
